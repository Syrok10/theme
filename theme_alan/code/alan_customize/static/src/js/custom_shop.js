odoo.define('alan_customize.customs_shop_js', function(require){
    ...
    $(document).on('click', '#load_more_products', function(){
        ...
        if(localStorage.getItem("product_view") == 'list')
            param_dict.list_view_active = '1';
        else
            param_dict.list_view_active = '0';
        ...
        $.get("/shop/load_next_products", param_dict).then(function(data){
            if(data.trim()){
                ...
                $btnEle.siblings("input[name='loaded_products_count']").val($("div#products_grid div.as-product-list > .row:first > div > form").length);
