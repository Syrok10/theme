...
class Web_Editor(Web_Editor):
    ...
    def save_scss(self, url, bundle_xmlid, content):
        ...
        custom_url = '/alan_customize/static/src/scss/options/colors/color_picker'+str(request.website.id)+'.scss'
...
class WebsiteSale(WebsiteSale):
    ...
    def variant_change_images(self, product_id, **kw):
        ret = {'value_ret': 0}
        if product_id:
            product_rec =  http.request.env['product.product'].sudo().search([('id','=',product_id)])
            if product_rec:
                values = {
